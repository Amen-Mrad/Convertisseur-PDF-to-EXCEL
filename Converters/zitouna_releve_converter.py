...
class ZitounaReleveConverter:
    ...
    def convertir(self):
        ...
        if not path or not os.path.exists(path):
            print("⚠️ Attention: PDF manquant - Veuillez choisir un fichier PDF ZITOUNA.")
            return
        ...
        if not rows:
            print("❌ Erreur: Aucune transaction - Impossible d'extraire des transactions du relevé ZITOUNA.")
            return
        ...
        def _clean_zitouna_amount(amount_str: str) -> str:
            ...
            try:
                ...
                if ',' in cleaned:
                    ...
                    # Formater la partie entière avec séparation des milliers (espaces)
                    # Exemple: "4000" -> "4 000", "25000" -> "25 000"
                    ...
                    # Retourner le format Zitouna standard avec séparation des milliers
                    result = f"{integer_part},{formatted_decimal}"
                    return f"-{result}" if is_negative else result
                else:
                    # Pas de virgule trouvée, ajouter ,000
                    # Formater la partie entière avec séparation des milliers
                    if len(cleaned) > 3:
                        formatted_integer = ''
