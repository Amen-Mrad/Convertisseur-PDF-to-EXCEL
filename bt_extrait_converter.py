...
class BtExtraitConverter:
    def __init__(self, root):
        self.root = root
        self.root.title("Convertisseur EXTRAT BT vers Excel")
        self.root.geometry("600x420")
        self.root.configure(bg='#f0f0f0')

        self.pdf_path = tk.StringVar()
        self.excel_name = tk.StringVar(value=f"EXTRAT_BT_{datetime.now().strftime('%Y%m%d_%H%M%S')}")

        self.setup_ui()

    def setup_ui(self):
        tk.Label(self.root, text="Convertisseur EXTRAT BT", font=("Arial", 18, "bold"), bg="#f0f0f0", fg="#2c3e50").pack(pady=(20,4))
        ...
        tk.Label(frm, text="Fichier PDF EXTRAT BT:", bg="#f0f0f0", fg="#2c3e50", font=("Arial", 11, "bold")).grid(row=0, column=0, sticky='w', pady=5)
        tk.Entry(frm, width=48, textvariable=self.pdf_path).grid(row=1, column=0, padx=(0,10))
        tk.Button(frm, text="Parcourir", command=self.browse_pdf, bg="#1e40af").grid(row=1, column=1)
    ...
    def browse_pdf(self):
        path = filedialog.askopenfilename(title="Choisir un PDF BT EXTRAT", filetypes=[["PDF", "*.pdf"]])
        if path:
            ...
            try:
                ...
                # Vérifier si c'est un extrait BT
                if detection_result['type'] == 'extrait_bt':
                    self.pdf_path.set(path)
                    base = os.path.splitext(os.path.basename(path))[0]
                    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
                    self.excel_name.set(f"EXTRAT_BT_{base}_{ts}")
                    
                    # Message de confirmation discret
                    print(f"✅ Document détecté: EXTRAT BT (Confiance: {detection_result['confidence']:.1%})")
                    ...
                    response = messagebox.askyesno(
                        ...
                        f"Ce convertisseur est conçu pour les extraits BT.\n"
    ...
    def convertir(self):
        ...
        if not path or not os.path.exists(path):
            messagebox.showwarning("PDF manquant", "Veuillez choisir un fichier PDF BT EXTRAT.")
            return
        
        try:
            print(f"DEBUG BT EXTRAT - PDF sélectionné: {path}")
        except Exception:
            pass
        
        rows = self.parse_pdf(path)
        if not rows:
            messagebox.showerror("Aucune transaction", "Impossible d'extraire des transactions de l'extrait BT.")
            return
        
        df = pd.DataFrame(rows, columns=["date", "libelle", "debit", "credit"])
        downloads = os.path.join(os.path.expanduser("~"), "Downloads")
        out = os.path.join(downloads, f"{self.excel_name.get().strip() or 'EXTRAT_BT'}.xlsx")
    ...
    def parse_pdf(self, pdf_path: str):
        ...
        # Pour EXTRAIT BT : dates complètes DD/MM/YYYY
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                ...
                # 2. Parser les transactions avec le format EXTRAIT BT
                for i, line in enumerate(lines):
                    print(f"DEBUG: Analyse ligne {i+1}: {line}")
                    
                    # Pour EXTRAIT BT : chercher une date complète au début de la ligne
                    date_match = re.match(r'^(\d{1,2}/\d{1,2}/\d{4})\s+', line)
                    
                    if date_match:
                        try:
                            # Date complète disponible directement pour EXTRAIT BT
                            date_operation = date_match.group(1)
                            print(f"DEBUG: Date extraite: {date_operation}")
                            
                            # Parser les colonnes EXTRAIT BT de manière plus précise
                            # Format: OPÉRATION | LIBELLÉ | VALEUR | DÉBIT | CRÉDIT
                            ...
                            for j, part in enumerate(parts[1:], 1):  # Ignorer la date d'opération
                                ...
                                # Vérifier si c'est un montant (colonne DEBIT ou CREDIT)
                                    ...
                                    # Vérifier si ce n'est pas un élément du libellé à exclure
                                    is_excluded = (
                                        cheque_pattern.match(part) or 
                                        virement_pattern.match(part) or
                                        date_pattern.match(part) or
                                        year_pattern.match(part) or
                                        day_pattern.match(part) or
                                        reference_pattern.match(part)
                                    )
                                    ...
                                    if not is_excluded:
                                        ...
                                        print(f"DEBUG: Élément du libellé exclu: {part}")
                                    in_libelle = False
    ...
    def _normalize_date_str(self, raw: str) -> str:
        ...
        Pour EXTRAIT BT : gère les formes DD/MM/YYYY directement.
        """
        if not raw:
            return ''
        try:
            txt = str(raw).strip()
            # Pour EXTRAIT BT : chercher le format DD/MM/YYYY
