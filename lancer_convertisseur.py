...
class ModernStyles:
    ...
    ZITOUNA = '#16a34a'
    STB = '#1E3A8A'  # Bleu STB
    BT = '#1e40af'   # Bleu Banque de Tunisie
...
class ConvertisseurLauncher:
    def __init__(self, root):
        ...
        # Etat de sélection
        self.selected_bank = None  # 'biat' | 'amen' | 'zitouna' | 'bna' (QNB et BTK masqués)
    ...
    def create_ui(self):
        ...
        bank_configs = [
            ...
            ('stb', 'STB BANK', '#1E3A8A', '#FFFFFF'),  # Bleu STB
            ('bt', 'BANQUE DE TUNISIE', '#1e40af', '#FFFFFF')  # Bleu BT
            # UBCI et UIB masqués car pas prêts
        ]
        
        for i, (bank_id, text, bg_color, fg_color) in enumerate(bank_configs):
            row = i // 3  # Grille 3 colonnes pour 6 banques
            col = i % 3
    ...
    def select_bank(self, bank_key):
        ...
        for key, btn in self.bank_buttons.items():
            ...
            if key == 'biat':
                ...
                btn.configure(bg='#1E3A8A', fg='#FFFFFF', relief='flat', borderwidth=0)
            elif key == 'bt':
                btn.configure(bg='#1e40af', fg='#FFFFFF', relief='flat', borderwidth=0)
            elif key == 'ubci':
                btn.configure(bg='#0ea5e9', fg='#FFFFFF', relief='flat', borderwidth=0)
            elif key == 'uib':
                btn.configure(bg='#ef4444', fg='#FFFFFF', relief='flat', borderwidth=0)
            else:
        ...
        if bank_key == 'biat':
            ...
            # STB: Relevé et Extrait disponibles
            self.rb_extrait.configure(state='normal')
        elif bank_key == 'bt':
            # BT: Seulement Relevé disponible (Extrait bloqué)
            self.rb_extrait.configure(state='disabled')
            if self.selected_type.get() == 'extrait':
                self.selected_type.set('')
            print("DEBUG: BT sélectionné - Extrait bloqué")
        # UBCI et UIB masqués: aucun réglage spécifique
        else:
            # Pour les autres banques, s'assurer que le bouton Relevé est visible
            try:
    ...
    def demarrer(self):
        ...
        route = {
            ...
            # ('bt', 'extrait'): 'bt_extrait_converter.py',  # Extrait BT bloqué
            ('ubci', 'extrait'): 'ubci_extrait_converter.py',
            ('uib', 'releve'): 'uib_releve_converter.py',
        }
    ...
    def lancer_releve_stb(self):
        ...
        try:
            if os.path.exists("stb_releve_converter.py"):
                subprocess.Popen([sys.executable, "stb_releve_converter.py"])
                self.root.destroy()
            else:
                messagebox.showerror("Erreur", "Fichier stb_releve_converter.py non trouvé")
        except Exception as e:
